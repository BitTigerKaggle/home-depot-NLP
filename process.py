...
def spell_check(s,spell_check_dict):
    if s in spell_check_dict.keys():
        print spell_check_dict[s]
...
jungle = set(stopwords.words('english'))
...
def str_stem(s):
    if isinstance(s, str):
        ...
        s = re.sub(r"([\W_])([a-z])", r"\1 \2", s)
        s = re.sub(r"([a-z])([\W_])", r"\1 \2", s)
        s = re.sub(r"([\W_])([0-9])", r"\1\2", s)  # \ 18 -> 1 \18
        s = re.sub(r"([0-9])([\W_])", r"\1\2", s)  # 1 \ -> 1\
        s = re.sub(r"([0-9])( *)-( *)([0-9])", r"\1 xbi \4", s) # "1 - 18" --> "1 xbi 18"
        s = re.sub(r"([0-9])( *)\.( *)([0-9])", r"\1.\4", s)  # "price9.3dollars" -> "price 9.3 dollars"
        s = re.sub(r"([0-9])( *),( *)([0-9])", r"\1 \4", s)  # "2,500" -> "2500"
        # s = re.sub(r"(^\.|/)", r"", s)
        # s = re.sub(r"(\.|/)$", r"", s)

        s = re.sub(r"[(){}<>]", "", s)  # remove unnecessary punctuation
        # separate word and digit
        s = re.sub(r"([0-9])([a-z])", r"\1 \2", s)
        s = re.sub(r"([a-z])([0-9])", r"\1 \2", s)

        # replace "\" in the words with space
        s = re.sub(r"([a-z])( *)\.( *)([a-z])", r"\1 \4", s)

        # change all the symbols represent "and" meaning to "xbi"
        s = s.replace(" x ", " xbi ")
        s = s.replace("*", " xbi ")
        s = s.replace(" by ", " xbi ")
        ...
        # remove "-" if not "1-18": where "1-18" has already been changed to "1 xbi 18"
        s = s.replace(" - ", " ")
        # remove period amd comma
        s = s.replace(" . ", " ")
        s = s.replace(",", " ")

        stops = set(stopwords.words("english"))
        s = " ".join([z for z in s.split(" ") if z not in stops])
        s = " ".join([str(strNum[z]) if z in strNum else z for z in s.split()])

        pattern = re.compile(r"^[a-z]+$")

        try:
            s = " ".join([stemmer.stem(word) if re.match(pattern, word) else word for word in s.split()])
        except:
            print 'error 1'


        try:
            s = " ".join([str(normalize_fraction(Decimal(word))) if len(re.findall("\d+\.\d+", word)) > 0\
...
def addLength(df_all):
    ...
    df_all['len_of_bullet'] = df_all['bullet'].map(lambda x: len(str(x).split())).astype(np.int64)
    df_all['len_of_attr'] = df_all['attr_text'].map(lambda x: len(str(x).split())).astype(np.int64)
    return df_all

###########################################################################################
# --------------------------------------------Main----------------------------------------#
train = pd.read_csv("train.csv", encoding="ISO-8859-1")
train['product_title'] = train['product_title'].map(lambda x: onlyascii(x))
train['search_term'] = train['search_term'].map(lambda x: onlyascii(x))
train['search_term'] = train['search_term'].map(lambda x: spell_check(x,spell_check_dict))

test = pd.read_csv("test.csv", encoding="ISO-8859-1")
test['product_title'] = test['product_title'].map(lambda x: onlyascii(x))
test['search_term'] = test['search_term'].map(lambda x: onlyascii(x))
test['search_term'] = test['search_term'].map(lambda x: spell_check(x,spell_check_dict))
print "spell check finished."

des = pd.read_csv("product_descriptions.csv", encoding="ISO-8859-1")
des['product_description'] = des['product_description'].map(lambda x: onlyascii(x))

brand = pd.read_csv("brand.csv", encoding="ISO-8859-1").ix[:, 1:]
brand['brand'] = brand['brand'].map(lambda x: onlyascii(x))

material = pd.read_csv("material2.csv", encoding="ISO-8859-1").ix[:,1:]
material['material'] = material['material'].map(lambda x: onlyascii(x))

attr = pd.read_csv("attr1.csv", encoding="ISO-8859-1").ix[:,1:]#no need remove non ascii


bullet = pd.read_csv("bullet.csv", encoding="ISO-8859-1").ix[:,1:]
bullet['bullet'] = bullet['bullet'].map(lambda x: onlyascii(x))

print "merge the datasets"
train = pd.DataFrame.merge(train, des, on='product_uid', how='left')
train = pd.DataFrame.merge(train, brand, on='product_uid', how='left')
train = pd.DataFrame.merge(train, material, on='product_uid', how='left')
train = pd.DataFrame.merge(train, attr, on='product_uid', how='left')
train = pd.DataFrame.merge(train, bullet, on='product_uid', how='left')

test = pd.DataFrame.merge(test, des, on='product_uid', how='left')
test = pd.DataFrame.merge(test, brand, on='product_uid', how='left')
test = pd.DataFrame.merge(test, material, on='product_uid', how='left')
test = pd.DataFrame.merge(test, attr, on='product_uid', how='left')
test = pd.DataFrame.merge(test, bullet, on='product_uid', how='left')

train['search_term'] = train['search_term'].map(lambda x: str_stem(str(x)))
train['product_title'] = train['product_title'].map(lambda x: str_stem(str(x)))
train['product_description'] = train['product_description'].map(lambda x: str_stem(str(x)))
train['brand'] = train['brand'].map(lambda x: str_stem(str(x)))
train['material'] = train['material'].map(lambda x: str_stem(str(x)))
train['bullet'] = train['bullet'].map(lambda x: str_stem(str(x)))
train['attr_text'] = train['attr_text'].map(lambda x: str_stem(str(x)))

test['search_term'] = test['search_term'].map(lambda x: str_stem(str(x)))
test['product_title'] = test['product_title'].map(lambda x: str_stem(str(x)))
test['product_description'] = test['product_description'].map(lambda x: str_stem(str(x)))
test['brand'] = test['brand'].map(lambda x: str_stem(str(x)))
test['material'] = test['material'].map(lambda x: str_stem(str(x)))
test['bullet'] = test['bullet'].map(lambda x: str_stem(str(x)))
test['attr_text'] = test['attr_text'].map(lambda x: str_stem(str(x)))

print "add features of length"
train = addLength(train)
test = addLength(test)
print "save the data"
